...
const pool = require('./db');  // Importa o pool de conexões com o banco
...
function validarDadosCurso(nome, cargaHoraria) {
  if (!nome || !cargaHoraria) {
    return "Nome do curso e carga horária são obrigatórios.";
  }
  return null;
}
...
app.post("/cursos", async (req, res) => {
  const { nome, cargaHoraria } = req.body;  // Alteração para cargaHoraria
  const erro = validarDadosCurso(nome, cargaHoraria);  // Alteração para cargaHoraria
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO cursos (nome, carga_horaria) VALUES ($1, $2) RETURNING *",  // Alteração para carga_horaria
      [nome, cargaHoraria]  // Alteração para cargaHoraria
    );
...
app.put("/cursos/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "UPDATE cursos SET nome = $1, carga_horaria = $2 WHERE id = $3 RETURNING *",  // Alteração para carga_horaria
      [nome, cargaHoraria, req.params.id]  // Alteração para cargaHoraria
    );
    if (result.rows.length === 0) return res.status(404).json({ message: "Curso não encontrado." });
    res.json(result.rows[0]);
  } catch (err) {
